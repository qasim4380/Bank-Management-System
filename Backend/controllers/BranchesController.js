    ...
    try {
        ...
        if (!data || data.length === 0) {
            return res.status(404).send({
                success: false,
                message: 'No branch records found',
            });
        }
        ...
        res.status(200).send({
            ...
            totalBranches: data.length, // ✅ Count based on array length
            data: data
        });
...
const updateBranch = async (req, res) => {
    try {
        ...
        // Validate that BranchID is provided and is a valid format
        if (!BranchID || isNaN(BranchID)) {
            return res.status(400).send({
                success: false,
                message: "Valid BranchID is required in the URL",
            });
        }

        // Check if the BranchID exists in the database
        const checkQuery = "SELECT * FROM Branches WHERE BranchID = ?";
        const [checkResult] = await db.query(checkQuery, [BranchID]);

        // If no branch is found with the given BranchID
        ...
        // Validate that at least one field (BranchName, Location, or Phone) is provided
