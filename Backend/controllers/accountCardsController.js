...
const createCard = async (req, res) => {
  //===================
  const { AccountID, CardType, PIN, Name } = req.body;
  console.log(`ATM Card Requested by: ${Name}`);
  console.log(`AccountID: ${AccountID}`);
  console.log(`CardType: ${CardType}`);
  console.log(`PIN: ${PIN}`);

  if (!AccountID || !CardType || !PIN) {
    return res.status(400).send({
      success: false,
      message: 'AccountID, CardType, and PIN are required.',
    });
  }
  ...
  try {
    ...
    if (cardData[0].CardType.toLowerCase() !== CardType.toLowerCase()) {
      return res.status(404).json({
        success: false,
        message: `You don't have ${CardType} card.`,
      });
    }
...
const depositMoney = async (req, res) => {
  try {
    ...
    if (cardDetails[0].CardID === null) {
      return res.status(404).json({
        success: false,
        message: `Customer with ID ${CustomerID} don't have a Card.`,
      });
    }
    console.log(req.body);
...
const withdrawMoney = async (req, res) => {
  try {
    let { CustomerID, CardType, PIN, Amount } = req.body;
    ...
    console.log("CardType from request:", CardType);
    console.log("CardType from database:", cardDetails[0].CardType);
